  ...
  applyFilter(storyFilter) {
    if (!storyFilter || !storyFilter.source_name) {
      ...
      this.filteredStories = this.stories.filter((story) => 
        story.source_names.toLowerCase().includes(storyFilter.source_name.toLowerCase())
      );
    }
    this.render();
  }
  ...
  render() {
    ...
    this.container
      ...
      .html(
        (d) => `
            <h3>${d.title}</h3>
            <div class="date">${new Date(d.articles[0].date).toLocaleDateString('en-US', {
                year: 'numeric',
...
class StoryModal {
  ...
  renderArticles(articles) {
    ...
    articles.forEach((article) => {
      const item = container.append("div").attr("class", "article-item");
      ...
      item.append("div").attr("class", "article-info").html(`
                    <h4><a href="${
                      article.url
        ...
        article.title
                    ...
                    <div class="source-info">
                        ...
                        <span class="bias-tag" style="background:${
                          source_bias_colors[bias_mapping[article.source_bias]]
                        }">
                            ${article.source_bias}
                        </span>
                    </div>
...
class StoryTimeline {
  ...
  updateTimeline(currentDate) {
    this.timeline
      .attr("x1", this.xScale(currentDate))
  ...
  updateDataPoints() {
    ...
    points.exit().transition().duration(300).style("opacity", 0).remove();
  }
  ...
  updatePolarizationTrend(currentDate) {
    ...
    const trend = this.svg
      .selectAll(".polarization-trend")
    ...
    const indicator = this.svg
      .selectAll(".trend-indicator")
      .data([currentDate]);
  ...
  resetVisualization() {
    ...
    this.svg
      .selectAll(
        ".data-point, .distribution, .polarization-trend, .trend-indicator"
      )
      .remove();
...
class StoryVisualizerApp {
  ...
  async init(storyFilter) {
    try {
      await this.storyList.loadData("dist/israeli-palestinian-conflict.json", storyFilter);
    } catch (error) {
...
export default StoryVisualizerApp;
